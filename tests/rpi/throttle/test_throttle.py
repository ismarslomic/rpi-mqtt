...
from rpi.throttle.sensor import ThrottledSensor
...
@patch("rpi.throttle.sensor.subprocess.run")
def test_read_throttle_status_not_throttled(mock_run):
    ...
    # Call function
    throttled_status: SystemThrottleStatus = ThrottledSensor().read()
...
@patch("rpi.throttle.sensor.subprocess.run", side_effect=FileNotFoundError("vcgencmd not found"))
